...
function be_schema_social_get_settings() {
	...
	$cached   = be_schema_social_merge_defaults( $settings );
...
function be_schema_social_is_enabled() {
	...
	// Honor per-network toggles; keep legacy master "enabled" for backward compatibility.
	$og_on       = isset( $settings['social_enable_og'] ) && '1' === (string) $settings['social_enable_og'];
	$twitter_on  = isset( $settings['social_enable_twitter'] ) && '1' === (string) $settings['social_enable_twitter'];
	$master_flag = isset( $settings['enabled'] ) && '1' === (string) $settings['enabled'];

	return ( $og_on || $twitter_on || $master_flag );
}
